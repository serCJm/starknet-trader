...
export class logger {
	...
	private static previousLogLines: number = 0;
	...
	private static logWithLevel(
		...
		if (overwrite) {
			// Calculate the number of lines the previous log entry took up
			const lines = Math.ceil(
				(this.previousLogLines + styledLogEntry.length) /
					process.stdout.columns
			);
			// Move the cursor up by that many lines
			...
			process.stdout.write(styledLogEntry);
			// Save the length of the new log entry
			this.previousLogLines = styledLogEntry.length;
		} else {
			console[logMethod as "log" | "error" | "warn" | "info"](
				"\n" + styledLogEntry
			);
			this.previousLogLines = 0;
		}
